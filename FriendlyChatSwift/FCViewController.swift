...
class FCViewController: UIViewController, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var room5Button: UIButton!
    @IBOutlet weak var room4Button: UIButton!
    @IBOutlet weak var room3Button: UIButton!
        ...
        _authHandle = FIRAuth.auth()?.addStateDidChangeListener{(auth: FIRAuth,user:FIRUser?)in
            ...
            if let activeUser = user {
                ...
                if self.user != activeUser {
                    ...
                    self.usernameSettings.text = self.displayName
                }
            } else {
                // user must sign in
    ...
    func configureDatabase() {
        ...
        _refHandle = ref.child(chatroom).observe(.childAdded) { (snapshot: FIRDataSnapshot) in
    ...
    func hideInterface(state : Bool){
        ...
        messageTextField.isHidden = state
        sendButton.isHidden = state
        imageMessage.isHidden = state
    ...
    func hideRoomView(state : Bool){
        
        Room1Button.isHidden = state
        Room2Button.isHidden = state
        room3Button.isHidden = state
        room4Button.isHidden = state
        room5Button.isHidden = state
    ...
    @IBAction func ChangeRoom(_ sender: Any) {
        ...
        hideRoomView(state: true)
        hideInterface(state: false)
        
    }
    
    @IBAction func ChangeRoom2(_ sender: Any) {
        disconnect()
        chatroom="room2"
        reconnect()
    ...
    @IBAction func ChangeRoom3(_ sender: Any) {
        disconnect()
        chatroom="room3"
        reconnect()
    ...
    @IBAction func ChangeRoom4(_ sender: Any) {
        disconnect()
        chatroom="room4"
        reconnect()
    ...
    @IBAction func ChangeRoom5(_ sender: Any) {
        disconnect()
        chatroom="room5"
        reconnect()
    ...
    @IBAction func Settings(_ sender: Any) {
        ...
        hideInterface(state: true)
        hideRoomView(state: true)
        hideSettings(state: false)
    }
    ...
    @IBAction func RoomOverview(_ sender: UIButton) {
        hideRoomView(state: false)
        hideSettings(state: true)
        hideInterface(state: true)
    }
...
extension FCViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let messagesSnapshot: FIRDataSnapshot! = messages[indexPath.row]
        let message = messagesSnapshot.value as! [String:String]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        print("You selected row at\(message[Constants.MessageFields.text] ?? "[message]" )")
